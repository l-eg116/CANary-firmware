...
enum EmissionMode {WaitACK, NoACK, Loopback}
...
// ### HOME ###
// ############
enum HomeItems {None, Emit, Capture} //noms non contractuels
struct Home {
...
impl DisplayManager<Home> {
	pub fn right_or_left_pressed(&mut self) {
		match self.current_screen.selected_item { // first state is None, then alternate between Capture and Emit
...
// #############################
enum FrameEmissionSettingsItems {Bitrate, Mode, Ok}
...
impl DisplayManager<FrameEmissionSettings> {
	pub fn up_or_down_pressed(&mut self) {
		match self.current_screen.selected_item {
			FrameEmissionSettingsItems::Bitrate => self.current_screen.selected_item = FrameEmissionSettingsItems::Mode,
			FrameEmissionSettingsItems::Mode => self.current_screen.selected_item = FrameEmissionSettingsItems::Ok,
			FrameEmissionSettingsItems::Ok => self.current_screen.selected_item = FrameEmissionSettingsItems::Bitrate
		}
	}
